...
class LigaPartido(models.Model):
    ...
    # Creamos dos campos para poder calcular los partidos con una diferencia de goles mayor a 4
    victoria_extra = fields.Integer()
    derrota_extra = fields.Integer()
    ...
    # Función para sumar 2 goles a todos los equipos que hayan jugado en casa
    def golesCasa(self):
        for record in self:
            record.goles_casa = record.goles_casa + 2
            self.actualizoRegistrosEquipo()

    # Función para sumar 2 goles a todos los equipos que hayan jugado fuera de casa
    ...
    def actualizoRegistrosEquipo(self):
        ...
        for recordEquipo in self.env['liga.equipo'].search([]):
            ...
            recordEquipo.victoria_extra=0
            recordEquipo.derrota_extra=0
            ...
            for recordPartido in self.env['liga.partido'].search([]):  
                ...
                if recordPartido.equipo_casa.nombre==recordEquipo.nombre:
                    ...
                    if recordPartido.goles_casa>recordPartido.goles_fuera:
                        # si la diferencia de goles entre ambos equipos es igual o mayor a 4
                        if recordPartido.goles_casa - recordPartido.goles_fuera >= 4:
                            # sumamos 1 a victoria_extra
                            recordEquipo.victoria_extra=recordEquipo.victoria_extra + 1
                        else:
                            recordEquipo.victorias=recordEquipo.victorias+1
                    elif recordPartido.goles_casa<recordPartido.goles_fuera:
                        # si la diferencia de goles entre ambos equipos es igual o mayor a 4
                        if recordPartido.goles_casa - recordPartido.goles_fuera >= 4:
                            # sumamos 1 a las derrota_extra, y restamos 1 a las victorias
                            recordEquipo.derrota_extra=recordEquipo.derrota_extra + 1
                        else:
    ...
    # Sobreescribo el borrado (unlink)
    def unlink(self):
        # Borro el registro
        r = super(LigaPartido, self).unlink()
        # Actualizo la clasificación para que reste los puntos de los partidos eliminados
        self.actualizoRegistrosEquipo()
        return r
