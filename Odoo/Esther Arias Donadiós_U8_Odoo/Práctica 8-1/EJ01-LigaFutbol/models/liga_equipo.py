...
class LigaEquipo(models.Model):
    ...
    #Dato binario, para guardar un binario (en la vista indicaremos que es una imagen) con el escudo del equipo
    escudo = fields.Image('Escudo equipo',max_width=50,max_height=50)

    #Anyo de fundacion
    fecha_fundacion = fields.Date('Fecha fundación')

    #Campo con HTML (Sanitizado) donde se guarda la descripción del equipo
    ...
    victoria_extra=fields.Integer(default=0)
    derrota_extra=fields.Integer(default=0)
    ...
    @api.depends('victorias','empates', 'victoria_extra', 'derrota_extra')
    def _compute_puntos(self):
        for record in self:
            record.puntos = record.victorias * 3 + record.victoria_extra * 4 + record.empates - record.derrota_extra
